...
export function Staff() {
  ...
  const suspend = async (s: StaffAccount) => {
    const ok = await dialogs.confirm(`Suspender a conta de ${s.email}?`, {
  ...
  const unsuspend = async (s: StaffAccount) => {
    const ok = await dialogs.confirm(`Reativar a conta de ${s.email}?`, {
          ...
          return (
            <div key={s.id} className="card">
              <div className="card-head">
                <div className="who-avatar" style={{ width: 42, height: 42, borderRadius: 14 }}>
                  {(s.email[0] || '?').toUpperCase()}
                </div>
                <div className="grow">
                  <div className="card-title">{s.email}</div>
                  <div className="tag-row" style={{ marginTop: 6 }}>
                    <Badge tone={roleTone(s.role)}>{STAFF_ROLE_LABEL[s.role] || s.role}</Badge>
                    {s.is_suspended && <Badge tone="red">Suspensa</Badge>}
                  </div>
                </div>
...
function CreateStaffModal({
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await api.post('/admin/staff', { email: email.trim(), password, role });
      await onCreated();
    } catch (e) {
