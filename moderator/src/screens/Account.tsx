...
// Seção "Minha conta": tudo que o staff mexe na própria conta sem sair do
// painel — trocar a senha (sem passar pelo "esqueci minha senha") e a A2F.
// Staff no Daqui é só um User com `staff_role` definido, então usa os mesmos
// endpoints genéricos de A2F/senha de qualquer usuário. O QR é gerado no
// cliente a partir do otpauth:// que o backend devolve — o segredo não passa
// por serviço externo.
...
function AvatarCard() {
  ...
  const upload = async (file: File) => {
    ...
    setBusy(true);
    try {
      // O backend do Daqui recebe a imagem como data URL (mesmo endpoint que
      // o app usa no perfil), não como multipart.
      await api.post('/users/me/avatar', { image: await readAsDataUrl(file) });
      await refresh();
    } catch (e) {
    ...
    <div className="form-card">
      <header>
        ...
        <span className="muted">
          Aparece na sua conta em todo o Daqui e ao lado do seu nome na equipe.
        </span>
      </header>
