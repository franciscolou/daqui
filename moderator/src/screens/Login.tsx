...
export function Login() {
  ...
  // O backend responde 403 "acesso restrito" pra conta sem cargo de staff —
  // traduzimos pra uma mensagem que faz sentido nesta tela.
  const describe = (e: unknown, fallback: string) => {
    if (e instanceof NotStaffError) return e.message;
    const msg = errorMessage(e, fallback);
    return msg.includes('restrito') ? 'Esta conta não é moderadora.' : msg;
  };
  ...
  const submitCredentials = async () => {
    ...
    try {
      ...
      await signIn(data.access_token ?? '', base(), appUrl);
    } catch (e) {
      setError(describe(e, 'Falha no login'));
    } finally {
      setBusy(false);
    }
  };
  ...
  const submitTwofa = async () => {
    ...
    try {
      ...
      setError(describe(e, 'Código inválido'));
    } finally {
      setBusy(false);
    }
  };
  ...
  const submitForgot = async () => {
    ...
    try {
      ...
      setForgotMsg(
        'Se este e-mail estiver cadastrado, você receberá um link para redefinir a senha — ele abre o app Daqui.',
      );
    } catch (e) {
      ...
      <div className="auth-card">
            ...
            <div className="auth-fields">
              ...
              <Field label="E-mail">
                <input
                  ...
                  placeholder="moderador@daqui.com"
                  value={email}
        ...
        {step === 'forgot' && (
          <>
            <div className="auth-fields">
              <p className="muted">
                Informe seu e-mail. Se existir uma conta com esse e-mail, enviaremos um link para
                redefinir a senha.
              </p>
              <Field label="E-mail">
                <input
