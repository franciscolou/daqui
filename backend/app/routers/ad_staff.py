...
from app.controllers import ad_staff as staff
from app.schemas.ad_auth import TokenResponse
from app.schemas.ad_staff import StaffInviteInfo, StaffOut

# Painel de anúncios: gestão de contas de staff (Moderador/Administrador),
# restrita a Administrador/Owner (ver core/deps.py::get_current_ads_administrator)
# — exceto o par GET/POST "/invite" que resolve/aceita o convite, aberto
# (sem sessão: quem está aceitando ainda não tem conta).
admin_router = APIRouter(prefix="/ads-admin/staff", tags=["ads-admin"])
