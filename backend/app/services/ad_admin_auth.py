...
from app.core.security import (
    create_ads_2fa_ticket,
    create_ads_admin_access_token,
    create_ads_password_reset_token,
    decode_ads_2fa_ticket,
    decode_ads_password_reset_token,
    hash_password,
    verify_password,
)
...
def login(db, payload: LoginRequest) -> LoginResponse:
    ...
    if admin.totp_enabled:
        ...
        return LoginResponse(requires_2fa=True, ticket=create_ads_2fa_ticket(admin.id))
    return LoginResponse(access_token=create_ads_admin_access_token(admin.id))


def login_2fa(db, payload: TwoFactorLoginRequest) -> TokenResponse:
    try:
        admin_id = int(decode_ads_2fa_ticket(payload.ticket))
    ...
    if admin.is_suspended:
        raise HTTPException(status_code=403, detail="Conta suspensa. Fale com um administrador.")
    return TokenResponse(access_token=create_ads_admin_access_token(admin.id))
...
def start_2fa_setup(db, admin: AdAdmin) -> TwoFactorSetupResponse:
    ...
    return TwoFactorSetupResponse(
        secret=secret,
        otpauth_url=totp.provisioning_uri(secret, account=admin.email, issuer="Daqui Ads"),
    )
...
def forgot_password(db, payload: ForgotPasswordRequest) -> None:
    admin = admin_dao.get_by_email(db, payload.email)
    if admin:
        token = create_ads_password_reset_token(admin.id)
...
def reset_password(db, payload: ResetPasswordRequest) -> None:
    try:
        admin_id = int(decode_ads_password_reset_token(payload.token))
    except (JWTError, ValueError):
        raise HTTPException(
