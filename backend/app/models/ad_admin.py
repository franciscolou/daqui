...
class AdAdmin(Base):
    ...
    Tabela própria, deliberadamente separada de `User` mesmo estando no mesmo
    banco: é um time interno mexendo com dinheiro/campanha, não um morador do
    app — login do painel de anúncios usa esta tabela (ver
    app/core/deps.py::get_current_ad_admin), com token isolado por `scope`
    (ver core/security.py) pra nunca valer como bearer de User nem vice-versa.
    Moderador/Administrador/Owner têm paridade operacional total (todos fazem
    tudo que o painel de anúncios oferece) — a única diferença de cargo é o
    escopo de gestão de contas (ver AdAdminRole/AD_ADMIN_RANK acima).
    """
    ...
    # por quem está acima no rank — ver services/ad_staff.py::admin_rename_staff.
